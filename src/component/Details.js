...
import brave from "../asset/Brave.jpg"
...
console.log(videoId);
  ...
  async function getMovieRecommendations(traktMovieId) {
    try {
      const response = await fetch(`https://api.trakt.tv/${media}/${traktMovieId}/related?limit=25&extended=full`, {
        headers: {
          "Content-Type": "application/json",
          "trakt-api-version": "2",
          "trakt-api-key": process.env.TRAKT_API_KEY
  ...
  async function getPopularMoviePosters(tmdbId) {
    ...
    const response = await fetch(`https://imdb-com.p.rapidapi.com/title/get-images?tconst=${tmdbId}`, {
      method: "GET",
      headers: {
...
async function getCastWithImages() {
  try {
    ...
    const traktResponse = await fetch(`https://api.trakt.tv/${media}/${id}/people`, {
      headers: {
        "Content-Type": "application/json",
        "trakt-api-version": "2",
        "trakt-api-key": process.env.TRAKT_API_KEY
    ...
    const actors = await Promise.all(traktData.cast.map(async (actor) => {
      const wikiResponse = await fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${actor.person.name.replace(" ", "_")}`);
  ...
  return (
    ...
    {data.title ? (<>  <div className="tw-w-full tw-bg-black tw-text-white">
      {/* Hero Section */}
        ...
        <div className="tw-absolute tw-inset-0 tw-flex tw-flex-col tw-justify-end tw-px-4 tw-pb-16 md:tw-px-12 lg:tw-px-20 lg:tw-pb-24">
          <div className="tw-max-w-2xl">
            ...
            <div className="tw-flex tw-items-center tw-text-sm tw-text-gray-400 tw-mb-4 tw-flex-wrap">
              ...
              <span className="tw-mr-3 tw-border tw-border-gray-600 tw-px-1 tw-text-xs">TV-MA</span>
              ...
              <span>{data?.genres?.map((item, index) => (
                <span key={index} className="tw-ml-2">{item}</span>
              ))}</span>
            </div>
            ...
            <div className="tw-flex tw-flex-wrap tw-gap-3">
              <button className="tw-flex tw-items-center tw-bg-white hover:tw-bg-gray-200 tw-text-black tw-font-medium tw-px-6 tw-py-2 tw-rounded tw-transition-colors"
      ...
      <div className=" tw-bg-black">
        ...
        { < MovieCarousel trendmovies={recommendations} Section={"Recommended For You "} />}
    ...
    {trailerModalOpen && (
       ...
       <div className="tw-relative tw-w-full tw-max-w-4xl tw-mx-4 tw-rounded-lg tw-overflow-hidden tw-shadow-2xl">
         ...
         <div className="tw-bg-black tw-p-4">
           <h3 className="tw-font-bold tw-text-lg tw-text-white">{data?.title} - Official Trailer</h3>
